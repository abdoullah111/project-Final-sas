...
const { ajouterApprenant, validerResultat, normaliserNom, recherchById, recherchByName, enregistrerResultat, calculerProgression,
FiltrerParNiveau, trierParProgression, afficherTableauDeBord, trierParNom, } = require('./progression.js');
    ...
    while (continuer){
     ...
     switch(choix){
        ...
        case "3": {
            ...
            const nomComplet = await poserQuestion("nomComplet: ");
        ...
        case "5":{
            ...
            const jourText = await poserQuestion("Jour (1-7): ")
            ...
            const totalExercicesText = await poserQuestion("total des exercices: ")
            const totalExercices = Number(totalExercicesText)

            const challengeText = await poserQuestion("Challenge terminé (oui/non): ")
            const challengeTermine = challengeText.toLowerCase() === "oui"
            const resultat = {
                jour,
                exercicesTermines,
                totalExercices,
